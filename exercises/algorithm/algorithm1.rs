        ...
        while ptrl.is_some() && ptrr.is_some() {
            unsafe {
                if ptrl.unwrap().clone().as_ref().val < ptrr.clone().unwrap().as_ref().val {
                    ...
                    list.add(ptrr.unwrap().as_ref().val.clone());
                    ptrr=ptrr.unwrap().as_ref().next;
                }
            }
        }
        while let Some(node) = ptrl {
            unsafe {
                list.add(node.as_ref().val.clone());
                ptrl = node.as_ref().next;
            }
        }

        while let Some(node) = ptrr {
            unsafe {
                list.add(node.as_ref().val.clone());
                ptrr = node.as_ref().next;
            }
        }
        list
    }
